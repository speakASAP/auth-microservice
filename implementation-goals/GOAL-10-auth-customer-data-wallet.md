...
- `[MISSING: owner-approved Cliplot bounded live checkout submit/live commerce window, flags, one-time inputs, duplicate-check rules, side-effect permission, and wallet-fetch freshness decision]`
- `[MISSING: owner-approved Rent-a-box route/onboarding migration window, route ownership list, feature-flag policy, onboarding decision, and later backfill/product-code migration plan before replacing local auth]`
