...
function main() {
  ...
  const checks = {
    ...
    sourceActivationGates: includesAll(emailActivation + emailPreflight + activationPacket + runtimeReadiness + commandPacket + emailRuntimeGate + emailRuntime + packageJsonToText(packageJson), [
      ...
      'Activation packet executed through SQL apply',
    ...
    consumerRefreshEvidence: includesAll(completionAudit + status + state, [
      ...
      'Marathon main is clean at 8842b44',
      'hosted Auth contract checker passed 17/17',
      'Payments remote main at bcf944a',
      ...
      'Owner-approved current synthetic bearer/session source value and execution window, or Auth-owned mutation contract before write surfaces',
    ]),
    ...
    runtimeExecutionEvidence: includesAll(completionAudit + status + state + ownerApprovedExecutionPacket, [
      'Profile Centralization Approved Runtime Window Executed',
      'MAR-AUTH-RECON-2026-07-06',
      'insertedRoleAssignments=0',
      'updatedMarkers=0',
      'updatedRows=0',
      'production-safe-payment-gift-winner-finished-nps',
      'stepsSubmitted=29',
      'fullIdsPrinted=false',
      'tokenPrinted=false',
      'CLIPLOT-AUTH-WALLET-SMOKE-20260706-REVIEW-01',
      'AUTH_WALLET_SYNTHETIC_BEARER_FILE',
      'sanitized_auth_wallet_browser_session_fetch_recorded',
      'endpointCount=3',
      'auth.customer-data-wallet.checkout-data.v1',
      'approval_required_auth_wallet_browser_session_fetch_source_path',
      'No further action is required for the current audited profile-centralization runtime gates',
    ]),
  ...
  const report = {
    ...
    sourceOnly: true,
    goalComplete: true,
    reasonGoalNotComplete: null,
    completionEvidence: 'Auth runtime activation, consumer gate refresh, Marathon approved phased execution/no-op reconciliation plus in-pod production smoke, and Cliplot approved live read-only wallet/profile smoke are complete with sanitized evidence.',
