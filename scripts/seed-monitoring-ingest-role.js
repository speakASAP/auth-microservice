#!/usr/bin/env node
/**
 * Creates the `internal:monitoring-microservice:ingest` role, idempotently.
 *
 * Needed by MonitoringIngestGuard (alert fire/resolve + credential self-report).
 * `admin`/`operator` already exist but are too broad for machine ingest writers.
 *
 * Dry run:
 *   kubectl exec -n statex-apps deploy/auth-microservice -c app -- \
 *     node scripts/seed-monitoring-ingest-role.js
 *
 * Apply:
 *   kubectl exec -n statex-apps deploy/auth-microservice -c app -- \
 *     node scripts/seed-monitoring-ingest-role.js --apply \
 *     --confirm-db-mutation=SERVICE_PRINCIPAL
 */

const CONTRACT = 'monitoring-ingest-role-seed.v1';
const DB_CONFIRMATION = 'SERVICE_PRINCIPAL';

const APP_NAME = 'monitoring-microservice';
const ROLE_SCOPE = 'internal';
const ROLE_NAME = 'ingest';
const ROLE_DESCRIPTION =
  'Write alert fire/resolve and credential self-report events to monitoring-microservice';
