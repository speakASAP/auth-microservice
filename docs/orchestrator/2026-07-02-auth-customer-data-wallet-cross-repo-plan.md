...
- `cliplot`: clean at `ea6cd93`, but checkout mutation remains guarded and
  approval-gated; runtime wallet integration is absent. Response fields and
  stable checkout-data version `auth.customer-data-wallet.checkout-data.v1`
  are now Auth source-defined; Cliplot-owned docs/verifiers still need refresh.
...
| C1 Cliplot plan                                        | source-facts-recorded; runtime-gated | Cliplot coordinator | Docs/guarded plan only | Selector/session/PII approvals and Cliplot docs/verifier refresh to consume Auth-defined schema version | Cliplot owner | After live checkout approval |
