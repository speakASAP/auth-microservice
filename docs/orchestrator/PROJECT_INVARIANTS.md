...
| AUTH-INV-003 | contract | `docs/UNIFIED_AUTH_CONTRACT.md` (human/user); `docs/SERVICE_IDENTITY_CONSUMER_STANDARD.md` (machine identity SPOT) | Human JWT/RBAC/API/OAuth/magic-link/redirect/CORS contracts and machine identity per SPOT remain compatible unless the plan names migration and validation steps. | Silent breaking change to user token claims, endpoint behavior, redirect safety, CORS, or machine identity; header/API-key S2S is not a valid protocol. | Contract impact section and targeted verification commands. | pre-coding, readiness |
