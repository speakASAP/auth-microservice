2026-06-29: Prepared approval-gated Catalog-to-Warehouse Auth service-principal provisioning source. Added `scripts/provision-catalog-warehouse-service-token.ts`, which supports a non-mutating `--dry-run` and an explicit `--apply` path guarded by `--confirm-db-mutation=CATALOG_WAREHOUSE_SERVICE_PRINCIPAL`, `--confirm-token-issuance=CATALOG_WAREHOUSE_SERVICE_JWT`, `--create-if-missing`, and `--token-output`. The helper creates/reuses only a `userType=service` principal, assigns `internal:warehouse-microservice:admin`, and writes the JWT only to the requested file with mode `0600`; it never prints token values. Auth `/auth/validate` now exposes service actor fields for `userType=service` users with `perApplicationPreferences.serviceIdentity.serviceName`, enabling Warehouse to classify Auth-compatible service JWTs as service actors. No helper execution, role assignment, service-principal creation, token issuance, Vault/Kubernetes secret mutation, deployment, database mutation, or token value inspection was performed by this source update.
