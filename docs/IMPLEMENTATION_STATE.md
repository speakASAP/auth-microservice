2026-07-02: Source-only Auth validation/logging-loop remediation completed on `alfares`. Runtime logs showed password login successes, while repeated `/auth/validate` failures came from a JWT subject `warehouse-reservation-expiry-cron` being treated as a UUID database id, producing Postgres `invalid input syntax for type uuid` errors. Auth now rejects non-UUID JWT subjects before user lookup, keeps expected validation denials as Unauthorized warnings instead of re-logging them as unexpected errors, and suppresses external audit emission for successful `validate_token` checks to avoid Auth -> Logging -> Auth validation feedback noise. Focused regression coverage was added in `src/auth/auth-contract.spec.ts`. Validation passed: `npm test -- --runTestsByPath src/auth/auth-contract.spec.ts` (10 tests), `git diff --check`, `npm run build`, and `npm run lint`. No production deployment, database mutation, raw production user-data read, token/secret/password/JWT value inspection, JWT payload shape change, RBAC/OAuth/magic-link/CORS/internal-service/database schema change, or consumer-service source edit was performed. Production deployment remains owner-approval gated.
