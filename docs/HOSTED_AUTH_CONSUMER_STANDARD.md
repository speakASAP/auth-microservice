...
| `payments-microservice` | `https://payments.alfares.cz/auth/callback` | Payments admin Lane C source audit; first-pass admin role contract uses `global:superadmin` and `internal:payments-microservice:admin` only; 2026-07-06 public revalidation returned HTTP 200 `valid:true` after a transient 503 window | verification lane; authenticated smoke requires owner-approved admin session packet |
